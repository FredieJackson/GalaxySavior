...
public class ConnectedThread extends Thread
{
    ...
    public void CheckAttack()
    {
        if(recievedData.startsWith(HandlerMSG.ATTACK))
        {
            ...
            Enemy.target.x = Integer.parseInt(tmp[1]);
            Log.i("CONNECTED.ATK_TARGET.X", "" + Enemy.target.x);
            Enemy.target.y = Integer.parseInt(tmp[2]);
            Log.i("CONNECTED.ATK_TARGET.Y", "" + Enemy.target.y);
            Enemy.damage = Integer.parseInt(tmp[3]);
            Log.i("CONNECTED.ATK_DAMAGE", "" + Enemy.damage);
            Enemy.weaponType = Byte.parseByte(tmp[4]);
            Log.i("CONNECTED.ATK_WEAPON_TYPE", "" + Enemy.weaponType);
        }
    }
    public void CheckAttackResult()
    {
        if(recievedData.startsWith(HandlerMSG.ATTACK_RESULT))
        {
            ...
            if(tmp.length > 3)
                return;
            Enemy.attackResult = Byte.parseByte(tmp[1]);
            Log.i("CONNECTED.ARSLT", "" + Enemy.attackResult);

            if(tmp.length != 3)
                return;
            Enemy.attackResultData = new String(tmp[2]);
            Log.i("CONNECTED.ARSLT_DATA", "" + Enemy.attackResultData);
        }
    }
}
