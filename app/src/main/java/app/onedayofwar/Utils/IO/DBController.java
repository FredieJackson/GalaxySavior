package app.onedayofwar.Utils.IO;
...
import app.onedayofwar.Game.Campaign.CharacterControl.Character;
...
public class DBController
{
    ...
    public void LoadWorld(GameScreen gameScreen)
    {
        Cursor c = db.query("WORLD", null, null, null, null, null, null);
        if (c.moveToFirst())
        {
            ...
            gameScreen.viewMatrix[12] = Float.parseFloat(coords[0]);
            gameScreen.viewMatrix[12] = Float.parseFloat(coords[1]);
            c.close();
        }
    }

    public void LoadPlanets(Loader loader, ArrayList<Planet> planets)
    {
        ...
        if (c.moveToFirst())
        {
            ...
            for(int i = 0; i < c.getCount(); i++)
            {
                ...
                planet.loadToMap(loader, c.getString(c.getColumnIndex("Skin")), coords);
                planet.DBLoad((byte)c.getInt(c.getColumnIndex("Size")), gArmy, sArmy, buildings, resources, rCapacity, factory, building, c.getInt(c.getColumnIndex("Owner")));
                c.moveToNext();
            }
        }
        c.close();
    ...
    public void LoadCharacter(boolean isPlayer, Character character)
    {
        ...
        if (c.moveToFirst())
        {
            ...
            character.DBLoad(Float.parseFloat(coords[0]), Float.parseFloat(coords[1]), c.getInt(c.getColumnIndex("MPoints")), new int[]{Integer.parseInt(resources[0]), Integer.parseInt(resources[1]), Integer.parseInt(resources[2])}, gArmy, sArmy);
        }
        c.close();
    }
    ...
    public void SavePlanets(ArrayList<Planet> planets)
    {
        ...
        for(int i = 0; i < planets.size(); i++)
        {
            ...
            contentValues.put("Resources", bAstS(Arrays.toString(planet.getResources())));
            contentValues.put("RCapacity", bAstS(Arrays.toString(planet.getCapacities())));
            contentValues.put("Owner", planet.IsConquered()? 1 : 0);
    ...
    public void SaveCharacter(boolean isPlayer, Character character)
    {
        ...
        contentValues.put("GArmy", bAstS(Arrays.toString(character.getGArmy())));
        contentValues.put("SArmy", bAstS(Arrays.toString(character.getSArmy())));
