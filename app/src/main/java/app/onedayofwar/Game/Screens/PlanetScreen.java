...
import app.onedayofwar.Game.Campaign.Space.Planet;
import app.onedayofwar.Game.Campaign.Space.Space;
import app.onedayofwar.Game.Campaign.Space.UpgradeSystem;
...
public class PlanetScreen implements Screen
{
    ...
    public PlanetScreen(ScreenController screenController, Space space, Planet planet)
    {
    ...
    public void Initialize(Loader loader)
    {
        ...
        float unitCoeff = screenController.screenWidth * 0.9f /(Assets.robotImage.getWidth() + Assets.ifvImage.getWidth() + Assets.rocketImage.getWidth() + Assets.tankImage.getWidth() + Assets.turretImage.getWidth() + Assets.sonderImage.getWidth() + 5 * 30);
        unit = new Sprite(Assets.robotImage);
        unit.Scale(unitCoeff);
        buildInfoTextSize = 0.05f * screenController.screenHeight;
    ...
    public void Draw(GLRenderer renderer)
    {
        ...
        switch(state)
        {
            ...
            case BUILD:
                /*renderer.DrawText("БИРЖА: " + planet.getBuildings()[0], Assets.gsFont, upgradeBuilding[0].getMatrix()[12], upgradeBuilding[0].getMatrix()[13] - upgradeBuilding[0].height/2 - buildInfoTextSize/2, 0, Assets.gsColor, buildInfoTextSize, true);
                renderer.DrawText("НЕФТЯНАЯ СКВАЖИНА: " + planet.getBuildings()[1], Assets.gsFont, upgradeBuilding[1].getMatrix()[12], upgradeBuilding[1].getMatrix()[13] - upgradeBuilding[1].height/2 - buildInfoTextSize/2, 0, Assets.gsColor, buildInfoTextSize, true);
                renderer.DrawText("ШАХТЫ НАНОСТАЛИ: " + planet.getBuildings()[2], Assets.gsFont, upgradeBuilding[2].getMatrix()[12], upgradeBuilding[2].getMatrix()[13] - upgradeBuilding[2].height/2 - buildInfoTextSize/2, 0, Assets.gsColor, buildInfoTextSize, true);
                renderer.DrawText("ЗАВОД: " + planet.getBuildings()[3], Assets.gsFont, upgradeBuilding[3].getMatrix()[12], upgradeBuilding[3].getMatrix()[13] - upgradeBuilding[3].height/2 - buildInfoTextSize/2, 0, Assets.gsColor, buildInfoTextSize, true);
                renderer.DrawText("МАСТЕРСКАЯ: " + planet.getBuildings()[4], Assets.gsFont, upgradeBuilding[4].getMatrix()[12], upgradeBuilding[4].getMatrix()[13] - upgradeBuilding[4].height/2 - buildInfoTextSize/2, 0, Assets.gsColor, buildInfoTextSize, true);

                if(!planet.buildingUpgrade.IsFalse())
                {
                    renderer.DrawText("ИДЕТ СТРОИТЕЛЬСТВО: " + planet.upgradeName + " " + (planet.getBuildings()[(int)planet.buildingUpgrade.x] + 1) + " УРОВЕНЬ", Assets.gsFont, glView.getScreenWidth()/2, glView.getScreenHeight()/2, 0, Assets.gsColor, buildInfoTextSize, true);
                }*/
                break;
            ...
            case FACTORY:
                /*if(createType == 1)
                {
                   ...
                   renderer.DrawSprite(unit);
                   renderer.DrawText("x" + planet.getGroundGuards()[0], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                   unit.Move(unit.getWidth()/2, 0);

                   unit.setTexture(Assets.ifvImage);
                   unit.Move(unit.getWidth() / 2 + 30 , 0);
                   renderer.DrawSprite(unit);
                   renderer.DrawText("x" + planet.getGroundGuards()[1], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                   unit.Move(unit.getWidth()/2, 0);

                   unit.setTexture(Assets.rocketImage);
                   unit.Move(unit.getWidth() / 2 + 30 , 0);
                   renderer.DrawSprite(unit);
                   renderer.DrawText("x" + planet.getGroundGuards()[2], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                   unit.Move(unit.getWidth()/2, 0);

                   unit.setTexture(Assets.tankImage);
                   unit.Move(unit.getWidth() / 2 + 30 , 0);
                   renderer.DrawSprite(unit);
                   renderer.DrawText("x" + planet.getGroundGuards()[3], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                   unit.Move(unit.getWidth()/2, 0);

                   unit.setTexture(Assets.turretImage);
                   unit.Move(unit.getWidth() / 2 + 30 , 0);
                   renderer.DrawSprite(unit);
                   renderer.DrawText("x" + planet.getGroundGuards()[4], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                   unit.Move(unit.getWidth()/2, 0);

                   unit.setTexture(Assets.sonderImage);
                   unit.Move(unit.getWidth() / 2 + 30 , 0);
                   renderer.DrawSprite(unit);
                   renderer.DrawText("x" + planet.getGroundGuards()[5], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                   unit.Move(unit.getWidth()/2, 0);

                    if(!planet.unitGroundUpgrade.IsFalse())
                    {
                        //renderer.DrawText("ИДЕТ СОЗДАНИЕ:" + planet.unitUpgradeName + " " + (planet.getGroundGuards()[(int)planet.unitGroundUpgrade.x] + 1) + " УРОВЕНЬ", Assets.gsFont, glView.getScreenWidth()/2, glView.getScreenHeight()/2, 0, Assets.gsColor, buildInfoTextSize, true);
                    }
                }
                else if(createType == 2)
                {
                    ...
                    renderer.DrawSprite(unit);
                    renderer.DrawText("x" + planet.getSpaceGuards()[0], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                    unit.Move(unit.getWidth()/2, unit.getHeight()/2);

                    unit.setTexture(Assets.akiraImage);
                    unit.Move(unit.getWidth() / 2 + 30 , 0);
                    renderer.DrawSprite(unit);
                    renderer.DrawText("x" + planet.getSpaceGuards()[1], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                    unit.Move(unit.getWidth()/2, 0);

                    unit.setTexture(Assets.defaintImage);
                    unit.Move(unit.getWidth() / 2 + 30 , 0);
                    renderer.DrawSprite(unit);
                    renderer.DrawText("x" + planet.getSpaceGuards()[2], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                    unit.Move(unit.getWidth()/2, 0);

                    unit.setTexture(Assets.battleshipImage);
                    unit.Move(unit.getWidth() / 2 + 30 , 0);
                    renderer.DrawSprite(unit);
                    renderer.DrawText("x" + planet.getSpaceGuards()[3], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                    unit.Move(unit.getWidth()/2, 0);

                    unit.setTexture(Assets.bioshipImage);
                    unit.Move(unit.getWidth() / 2 + 30 , 0);
                    renderer.DrawSprite(unit);
                    renderer.DrawText("x" + planet.getSpaceGuards()[4], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                    unit.Move(unit.getWidth()/2,0);

                    unit.setTexture(Assets.birdOfPreyImage);
                    unit.Move(unit.getWidth() / 2 + 30 , 0);
                    renderer.DrawSprite(unit);
                    renderer.DrawText("x" + planet.getSpaceGuards()[5], Assets.gsFont, unit.matrix[12] + unit.getWidth() / 2, unit.matrix[13], 0, Assets.gsColor, buildInfoTextSize, false);
                    unit.Move(unit.getWidth()/2 ,0);

                    if(!planet.unitSpaceUpgrade.IsFalse())
                    {
                        renderer.DrawText("ИДЕТ СОЗДАНИЕ: " + planet.unitUpgradeName + " " + (planet.getSpaceGuards()[(int)planet.unitSpaceUpgrade.x] + 1) + " УРОВЕНЬ", Assets.gsFont, glView.getScreenWidth()/2, glView.getScreenHeight()/2, 0, Assets.gsColor, buildInfoTextSize, true);
                    }
                }*/
                break;
            ...
            case RESOURCES_TRANSACTION:
                //renderer.DrawText("Склад корабля: КРЕДИТЫ: " + space.getPlayer().getResources()[0] + " // НЕФТЬ: " + space.getPlayer().getResources()[1] + " // НАНОСТАЛЬ: " + space.getPlayer().getResources()[2], Assets.gsFont, glView.getScreenWidth()/2, buildInfoTextSize * 2, 0, Assets.gsColor, buildInfoTextSize, true);
                break;
        }
        ButtonsDraw(renderer);
       // renderer.DrawText("КРЕДИТЫ: " + planet.getResources()[0] + " // НЕФТЬ: " + planet.getResources()[1] + " // НАНОСТАЛЬ: " + planet.getResources()[2], Assets.gsFont, glView.getScreenWidth()/2, buildInfoTextSize/2f, 0, Assets.gsColor, buildInfoTextSize, true);
    }
    ...
    private void ButtonsDraw(GLRenderer renderer)
    {
        if(planet.IsConquered())
        {
            buildInfo.Draw(renderer);
            showFactory.Draw(renderer);
            resourcesMenuBtn.Draw(renderer);
            unitMenuBtn.Draw(renderer);
            exportBtn.Draw(renderer);
            importBtn.Draw(renderer);
            selectOil.Draw(renderer);
            selectCredits.Draw(renderer);
            selectNanosteel.Draw(renderer);
            groundArmyBtn.Draw(renderer);
            spaceArmyBtn.Draw(renderer);
            for (int i = 0; i < upgradeBuilding.length; i++)
            {
                upgradeBuilding[i].Draw(renderer);
            }
            for (int i = 0; i < createUnit.length; i++)
            {
                createUnit[i].Draw(renderer);
            }
        }
        else
        {
            attackBtn.Draw(renderer);
        }
    ...
    private void ButtonsInitialize()
    {
        ...
        attackBtn.SetPosition(screenController.screenWidth/2, screenController.screenHeight/2);

        exitBtn = new Button(Assets.btnQBack, 0, 0, false);
        exitBtn.Scale(Assets.btnCoeff);
        exitBtn.SetPosition(screenController.screenWidth - exitBtn.width, screenController.screenHeight - exitBtn.height);

        buildInfo = new Button(Assets.btnBuildings, 0, 0, false);
        buildInfo.Scale(Assets.btnCoeff);
        buildInfo.SetPosition(screenController.screenWidth/2 - buildInfo.width, screenController.screenHeight/2 - buildInfo.height);

        showFactory = new Button(Assets.btnFactory, 0, 0, false);
        showFactory.Scale(Assets.btnCoeff);
        showFactory.SetPosition(screenController.screenWidth/2 + showFactory.width, screenController.screenHeight/2 - showFactory.height);

        resourcesMenuBtn = new Button(Assets.btnResources, 0, 0, false);
        resourcesMenuBtn.Scale(Assets.btnCoeff);
        resourcesMenuBtn.SetPosition(screenController.screenWidth/2 - resourcesMenuBtn.width, screenController.screenHeight/2 + resourcesMenuBtn.height);

        unitMenuBtn = new Button(Assets.btnArmy, 0, 0, false);
        unitMenuBtn.Scale(Assets.btnCoeff);
        unitMenuBtn.SetPosition(screenController.screenWidth/2 + unitMenuBtn.width, screenController.screenHeight/2 + unitMenuBtn.height);
        ...
        exportBtn.SetPosition(screenController.screenWidth/2 - exportBtn.width, screenController.screenHeight/2 + exportBtn.height);
        exportBtn.Lock();
        exportBtn.SetInvisible();

        importBtn = new Button(Assets.btnImport, 0, 0, false);
        importBtn.Scale(Assets.btnCoeff);
        importBtn.SetPosition(screenController.screenWidth/2 + importBtn.width, screenController.screenHeight/2 + exportBtn.height);
        importBtn.Lock();
        importBtn.SetInvisible();

        selectCredits = new Button(Assets.btnCredits, 0, 0, false);
        selectCredits.Scale(Assets.btnCoeff);
        selectCredits.SetPosition(screenController.screenWidth/2 - 2 * exportBtn.width, screenController.screenHeight/2 - selectCredits.height);
        selectCredits.Lock();
        selectCredits.SetInvisible();

        selectOil = new Button(Assets.btnOil, 0, 0, false);
        selectOil.Scale(Assets.btnCoeff);
        selectOil.SetPosition(screenController.screenWidth/2, screenController.screenHeight/2 - selectOil.height);
        selectOil.Lock();
        selectOil.SetInvisible();

        selectNanosteel = new Button(Assets.btnNanosteel, 0, 0, false);
        selectNanosteel.Scale(Assets.btnCoeff);
        selectNanosteel.SetPosition(screenController.screenWidth/2 + 2 * exportBtn.width, screenController.screenHeight/2 - selectNanosteel.height);
        selectNanosteel.Lock();
        selectNanosteel.SetInvisible();

        groundArmyBtn = new Button(Assets.btnGArmy, 0, 0, false);
        groundArmyBtn.Scale(Assets.btnCoeff);
        groundArmyBtn.SetPosition(screenController.screenWidth/2 - groundArmyBtn.width, screenController.screenHeight/2);
        groundArmyBtn.SetInvisible();
        groundArmyBtn.Lock();

        spaceArmyBtn = new Button(Assets.btnSArmy, 0, 0, false);
        spaceArmyBtn.Scale(Assets.btnCoeff);
        spaceArmyBtn.SetPosition(screenController.screenWidth/2 + groundArmyBtn.width, screenController.screenHeight/2);
        spaceArmyBtn.SetInvisible();
        spaceArmyBtn.Lock();
    }
}
