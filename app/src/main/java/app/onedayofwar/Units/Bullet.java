...
public class Bullet
{
    ...
    public void Draw(Graphics g)
    {
        if(state == State.FLY)
        {
            g.drawSprite(image, matrix);//position.x - offset.x, position.y - offset.y);
    ...
    public void Update()
    {
        ...
        velocityY = (byte)((int)((float)destination.y / destination.x / destination.x * position.x * position.x) - position.y);
        curveAngle = (float)(Math.atan((double)2 * destination.y * position.x / destination.x / destination.x) * 180 / Math.PI);
        position.x += velocity;
        position.y += velocityY;
        matrix.postTranslate(velocity, velocityY);
